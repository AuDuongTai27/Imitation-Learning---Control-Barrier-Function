#!/usr/bin/env python3
"""
data_collection_sim_90.py
─────────────────────────
ROS 2 Node dùng để thu thập dữ liệu huấn luyện Imitation Learning trong môi trường mô phỏng (f1tenth_gym_ros).
Đã được cấu hình với 90 tia LiDAR bao phủ góc quét [-90, 90] độ (180 độ phía trước đầu xe).

Subscribe:
  - `/scan` (sensor_msgs/msg/LaserScan)
  - `/drive` (ackermann_msgs/msg/AckermannDriveStamped)

Format CSV: [lidar_0, lidar_1, ..., lidar_89, speed, steering_angle]
"""
...
class DataCollectionSim90Node(Node):
    def __init__(self):
        super().__init__('data_collection_sim_90_node')

        # --- 1. Parameters ---
        self.declare_parameter('dataset_path', '/sim_ws/src/pure_pursuit_controller/pure_pursuit_controller/dagger_dataset_sim_90.csv')
        self.declare_parameter('target_beams', 90)
        ...
        self.get_logger().info(" DATA COLLECTION SIM 90 BEAMS STARTED")
    ...
    def _write_header(self):
        ...
        self.get_logger().info("Created new simulation dataset CSV file with 90 beams headers.")
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        # Tiền xử lý scan (Crop & Downsample về 90 beams và [-90, 90] độ)
    ...
    def preprocess_scan(self, msg: LaserScan):
        """
        Crop góc quét LiDAR về [-90, 90] độ phía trước mặt (180 độ)
        và downsample về đúng self.target_beams (90 beams) bằng cách nội suy.
        """
        ...
        # Giới hạn góc quét [-90, 90] độ (radians)
        crop_limit = math.radians(90.0)

        # Tính toán góc tương ứng với từng điểm scan
        angles = np.arange(len(ranges)) * angle_increment + angle_min

        # Lọc ra các điểm nằm trong góc [-90, 90] độ
        ...
        # Nội suy (Interpolation) để có đúng target_beams (90 beams)
    ...
    def _flush_buffer(self, data_list):
        ...
        try:
            ...
            self.get_logger().info(f"[SIM 90] Flushed {len(data_list)} samples to CSV. Total samples: {self.total_saved_samples}")
        except Exception as e:
            self.get_logger().error(f"Error while flushing buffer to CSV: {e}")

def main(args=None):
    rclpy.init(args=args)
    node = DataCollectionSim90Node()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        node.get_logger().warn("Shutting down simulation data collection (90 beams) node.")
