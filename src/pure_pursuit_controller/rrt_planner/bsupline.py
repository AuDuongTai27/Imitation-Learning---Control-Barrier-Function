...
class ContinuousLocalPlanner(Node):
    def __init__(self):
        ...
        self.local_max_x = 3.5   
        self.local_min_y = -1.2  
        self.local_max_y = 1.2   
        self.local_res = 0.05    
        ...
        self.get_logger().info("RRT Spline Planner Started.")
    ...
    def control_loop(self):
        ...
        if found:
            ...
            # === [MỚI] ÁP DỤNG B-SPLINE LÀM MƯỢT ===
            # Biến đường gấp khúc thành đường cong mềm
            ...
            self.visualize(path_smooth, target_local) # Visualize đường cong
    ...
    # --- HÀM LÀM MƯỢT B-SPLINE (QUAN TRỌNG) ---
    def apply_b_spline(self, path_points):
        """ Làm mượt đường gấp khúc RRT thành đường cong """
        if len(path_points) < 3: return path_points # Cần ít nhất 3 điểm để uốn cong

        try:
            x = [p[0] for p in path_points]
            y = [p[1] for p in path_points]

            # Lọc điểm trùng (Spline không chịu được điểm trùng)
            x_u, y_u = [x[0]], [y[0]]
            ...
            # Tạo B-Spline representation
            # k=3 (Cubic), s=0.05 (Smoothing factor - càng lớn càng mượt nhưng càng cắt góc)
            tck, u = interpolate.splprep([x_u, y_u], k=3, s=0.05)
            
            # Sinh ra đường mới dày đặc hơn (Interpolation)
            u_fine = np.linspace(0, 1, num=50) # Sinh ra 50 điểm mịn
            ...
            self.get_logger().warn(f"Spline Error: {e}")
            return path_points
